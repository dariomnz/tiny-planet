...
Program::Program(const Shader &vert, const Shader &frag) {
  ...
  if (!ok) {
    ...
    glGetProgramInfoLog(m_id, static_cast<GLsizei>(log.size()), nullptr,
                        log.data());
...
Program::Program(std::string_view vertSrc, std::string_view fragSrc)
    : Program(Shader(ShaderType::Vertex, vertSrc),
              Shader(ShaderType::Fragment, fragSrc)) {}
...
Program::Program(Program &&other) noexcept
    : m_id(other.m_id), m_cache(std::move(other.m_cache)) {
  other.m_id = 0;
}
...
void Program::set(std::string_view name, float v) const {
  glUniform1f(uniformLocation(name), v);
}

void Program::set(std::string_view name, const glm::vec2 &v) const {
