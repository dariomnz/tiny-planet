...
Window::Window(Window &&other) noexcept : m_handle(other.m_handle), m_fbW(other.m_fbW), m_fbH(other.m_fbH) {
    other.m_handle = nullptr;
    if (m_handle) s_active = this;
}
...
void Window::onFramebufferSize(GLFWwindow * /*win*/, int w, int h) {
    if (s_active) s_active->setFramebufferSize(w, h);
}
